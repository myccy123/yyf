<template>
  <el-container style="height: 100%; border: 1px solid #eee;">
    ...
    <el-container>
      <el-aside :width="isCollapse?'64px':'200px'" style="background-color: rgb(238, 241, 246);text-align: left;">
        ...
        <el-menu router :unique-opened="true" :collapse="isCollapse" :collapse-transition="true">
          <el-submenu v-for="(item,index) in $router.options.routes" v-if="!item.hidden" :index="index+''" :key="index">
            <template slot="title">
              <i v-if="item.meta.icon" :class="item.meta.icon"></i><span>{{item.name}}</span>
            </template>
